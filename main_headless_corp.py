...
def setup_custom_logging():
    ...
    if not logger_sys.hasHandlers():
        sys_h = logging.FileHandler(f"logs/system/{today}_headless.log")
    ...
    if not logger_err.hasHandlers():
        err_h = logging.FileHandler(f"logs/error/error_{today}_headless.log")
    ...
    if not logger_att.hasHandlers():
        att_h = logging.FileHandler(f"logs/attendance/attendance_{today}_headless.log")
...
# ==========================================
# 3. CORE HEADLESS CONFIG & UTILITIES
# ==========================================
class HeadlessAttendanceCorp:
    def __init__(self):
        ...
        logger_sys.info("🧠 Memuat Model FaceNet dan MediaPipe (Headless Mode)...")
        ...
        logger_sys.info(f"Mencoba membuka kamera: {self.VIDEO_SOURCE}")
    ...
    def load_config(self):
        ...
        self.NAMA_MESIN = "HEADLESS-PC-CORP"
    ...
    def background_sync_worker(self):
        logger_sys.info("🔄 Background Sync Worker Corp harian berjalan.")
        while running_system:
            if self.SYNC_API_URL != "":
                try:
                    ...
                    logger_err.error(f"⚠️ Gagal sinkronisasi harian Headless: {e}")
            time.sleep(86400) 
    ...
    def kirim_data_ke_backend(self, nip, frame_crop):
        try:
            ...
            logger_err.error(f"❌ GAGAL memproses API Headless: {str(e)}")
            ...
            if filename.endswith((".jpg", ".png", ".jpeg")):
                ...
                if nip not in self.known_face_nips:
                    ...
                    try:
                        ...
                        pass
        logger_sys.info(f"✅ Load database sukses! {len(self.known_face_nips)} wajah aktif di memori Headless Corp.")

    def run(self):
        global running_system
        logger_sys.info("🚀 Mesin utama Headless Corp sukses mengudara...")
        ...
        while running_system:
                                    ...
                                    if len(self.known_face_encodings) > 0:
                                        ...
                                        if distances[best_match_index] < self.FACENET_THRESHOLD:
                                            ...
                                            if current_time - last_seen > self.COOLDOWN_DETIK:
                                                ...
                                                logger_att.info(f"[Headless Corp] Terdeteksi: {display_nip} - {display_name}")
            ...
            time.sleep(0.030) # Pembatasan FPS (~30 FPS) murni menghemat CPU Server

    def clean_exit(self):
        self.cap.release()
        logger_sys.info("✅ Kamera berhasil dilepas. Headless Corp mati dengan aman.")

def sigterm_handler(signum, frame):
    global running_system
    logger_sys.info("🛑 Menerima sinyal shutdown (SIGTERM/SIGINT). Menghentikan sistem...")
